...
function makeMockScene(overrides: Record<string, unknown> = {}) {
  const base = {
    ...
    walletManager: { getAiWallet: () => 100000 },
    aiMemoryManager: { getAiFirstRoundExtraBlocks: () => [], isAiMultiGameMemoryEnabled: () => false, getAiConversationMessages: () => [], getAiCrossGameMemoryCount: () => 0, getAiInGameHistoryCount: () => 0, pushAiRoundSummary: () => {} },
    aiIntelManager: { getAiResourceSnapshot: () => ({ skills: {}, items: {} }), buildAiActionConstraintBlock: () => ({}), processSingleAiIntelAction: vi.fn().mockResolvedValue(undefined), formatAiIntelActionPublicLine: () => "" },
