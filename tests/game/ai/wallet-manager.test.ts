...
describe("AiWalletManager", () => {
  describe("getAiWallet", () => {
    it("返回注入的 aiWallets 中的余额", () => {
      ...
      const manager = new AiWalletManager(players, aiWallets, () => makeCtx({ aiWallets }))
  ...
  describe("resetAiWallets", () => {
    it("重置所有 AI 钱包为初始值", () => {
      ...
      const manager = new AiWalletManager(players, aiWallets, () => makeCtx())
  ...
  describe("saveAiWalletsToStorage", () => {
    ...
    it("保存当前 aiWallets 到 localStorage", () => {
      const aiWallets = { "ai-1": 500000 }
      const manager = new AiWalletManager([], aiWallets, () => makeCtx())
  ...
  describe("normalizeAiBidValue", () => {
    it("出价超过钱包余额时截断", () => {
      ...
      const manager = new AiWalletManager([], aiWallets, () => makeCtx({ aiWallets }))
  ...
  describe("ctxProvider 动态读取", () => {
    it("ctxProvider 返回的动态值被正确使用", () => {
      const aiWallets = { "ai-1": 500000 }
      let currentBid = 1000
      const manager = new AiWalletManager([], aiWallets, () => ({
        ...makeCtx({ aiWallets }),
        currentBid,
      }))
