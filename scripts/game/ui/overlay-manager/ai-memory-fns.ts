/**
 * @file ai-memory-fns.ts
 * @module ui/overlay-manager/ai-memory-fns
 * @description AI 记忆面板操作函数（打开/关闭/触摸滚动绑定）
 */
...
export function openAiMemoryPanel(deps: UiOverlayManagerDeps, state: UiOverlayManagerState): void {
  ...
  if (aiPlayers.length === 0) {
    if (deps.dom.aiMemoryContent) {
      deps.dom.aiMemoryContent.innerHTML = '<div class="ai-memory-empty">暂无AI玩家</div>'
    }
  ...
  const sections = aiPlayers
    .map((player, idx) => {
      const memory = deps.ensureAiCrossGameMemory(player.id)
  ...
  if (deps.dom.aiMemoryContent) {
    deps.dom.aiMemoryContent.innerHTML = sections || '<div class="ai-memory-empty">暂无记忆数据</div>'
  }
  if (!state.aiMemoryTouchBound) {
    state.aiMemoryTouchBound = true
    setupAiMemoryTouchScroll(deps)
  }
  deps.dom.aiMemoryOverlay.classList.remove("hidden")
}

export function setupAiMemoryTouchScroll(deps: UiOverlayManagerDeps): void {
  const content = deps.dom.aiMemoryContent
  if (!content) return
  let touchStartY = 0
...
export function closeAiMemoryPanel(deps: UiOverlayManagerDeps): void {
  if (deps.dom.aiMemoryOverlay) {
    deps.dom.aiMemoryOverlay.classList.add("hidden")
  }
}
