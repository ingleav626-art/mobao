(function setupMobaoWarehouse(global) {
  ...
  const WarehouseRevealMixin = {
    ...
    renderQualityVisual(item, options = {}) {
      ...
      const existingScale = hasExistingImage ? { x: item.view.artifactImage.scaleX, y: item.view.artifactImage.scaleY } : null;
      ...
      const marker = this.add.rectangle(markerX + markerW / 2, markerY + markerH / 2, markerW, markerH, item.quality.color, hasArtifactImage ? 0.35 : 0.45);
