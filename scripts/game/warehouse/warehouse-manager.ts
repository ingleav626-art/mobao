...
export class WarehouseManager {
  ...
  preloadArtifactImages(): void { preloadArtifactImages(this.deps) }
  drawUnknownWarehouse(): void { drawUnknownWarehouse(this.deps) }
  drawGridLines(): void { drawGridLines(this.deps) }
  guardWarehouseCapacity(): void { guardWarehouseCapacity(this.deps) }
  spawnRandomItems(): void { spawnRandomItems(this.deps) }
  setupWarehouseAuction(): void { setupWarehouseAuction(this.deps) }
  ...
  rebuildWarehouseCellIndex(): void { rebuildWarehouseCellIndex(this.deps) }
  isInBoundsCell(x: number, y: number): boolean { return isInBoundsCell(x, y, GRID_COLS, GRID_ROWS) }
  isWarehouseCellOccupied(x: number, y: number): boolean { return isWarehouseCellOccupied(this.deps, x, y) }
  renderItem(item: Artifact): void { renderItem(this.deps, item) }
  onArtifactClicked(item: Artifact, pointer: { x: number; y: number }): void {
    onArtifactClicked(this.deps, item, pointer)
  }
  hasAnyInfo(item: Artifact): boolean { return hasAnyInfo(item) }
  getItemKnownText(item: Artifact): string { return getItemKnownText(item) }
  ...
  ): ReturnType<typeof revealArtifactFully> {
  ...
  playFullRevealEffect(item: Artifact): void { playFullRevealEffect(this.deps, item) }
  ...
  hideRevealScrollHints(): void { hideRevealScrollHints(this.deps) }
  ...
  refreshRevealScrollHints(): void { refreshRevealScrollHints(this.deps) }
  ...
  playOutlineRevealEffect(item: Artifact): void { playOutlineRevealEffect(this.deps, item) }
  playQualityRevealEffect(item: Artifact): void { playQualityRevealEffect(this.deps, item) }
  ...
  revealCell(col: number, row: number): void { revealCell(this.deps, col, row) }

  // ─── WarehousePreviewMixin 逻辑 ───

  positionPreview(canvasX: number, canvasY: number): void { positionPreview(this.deps, canvasX, canvasY) }
  applyPreviewPosition(): void { applyPreviewPosition(this.deps) }
  repositionPreview(): void { repositionPreview(this.deps) }
  hidePreview(): void { hidePreview(this.deps) }
  setupPreviewTouchScroll(): void { setupPreviewTouchScroll(this.deps) }
  isPointOnSettlementLockedItem(x: number, y: number): boolean {
    return isPointOnSettlementLockedItem(this.deps, x, y)
  }
  renderPreviewCandidates(item: Artifact): void { renderPreviewCandidates(this.deps, item) }
  renderSettlementItemPreview(item: Artifact): void { renderSettlementItemPreview(this.deps, item) }
}