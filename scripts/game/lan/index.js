...
export const LanIndexMixin = {
  initLanLobby() {
    ...
    const lanSlotConfig = [{ type: "empty" }, { type: "empty" }, { type: "empty" }, { type: "empty" }]
      ...
      if (cfg.type === "host") {
        ...
        const charAvatar =
          cfg.characterId && CharacterData
            ? getCharAvatarHtml(cfg.characterId)
    ...
    const bindSlotActions = (container) => {
      ...
      container.querySelectorAll("[data-add-ai]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          this.writeLog(
            `添加AI尝试 | human=${humanCount} | ai=${currentAiCount} | total=${humanCount + currentAiCount}/${maxPlayers}`
          )
          ...
          this.writeLog(
            `添加AI成功 | slot=${slotIdx} | name=AI-${aiIdx + 1} | total=${humanCount + currentAiCount + 1}/${maxPlayers}`
          )
          renderSlots()
          broadcastSlotState()
        })
    ...
    const openLanCarryItemPicker = () => {
      ...
      var renderPicker = function () {
        ...
        panel.querySelector(".carry-picker-close").addEventListener("click", function () {
