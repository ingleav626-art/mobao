...
export function initLanLobby(
  ...
  if (reconnectFailed) {
    ...
    deps.writeLog("检测到保存的房间数据 | room=" + savedRoomCode + " | player=" + savedPlayerId + " | host=" + savedIsHost)
  ...
  const renderLanPlayerSlot = (el: Element, i: number, cfg: Record<string, unknown>) => {
    ...
    if (cfg.type === "host") {
      ...
      const charAvatar = cfg.characterId && CharacterData ? getCharAvatarHtml(cfg.characterId as string) : '<span class="lan-avatar-emoji">👑</span>'
  ...
  const bindSlotActions = (container: HTMLElement) => {
    ...
    container.querySelectorAll("[data-add-ai]").forEach((btn) => {
      btn.addEventListener("click", (e: Event) => {
        ...
        deps.writeLog("添加AI尝试 | human=" + humanCount + " | ai=" + currentAiCount + " | total=" + (humanCount + currentAiCount) + "/" + maxPlayers)
        ...
        deps.writeLog("添加AI成功 | slot=" + slotIdx + " | name=AI-" + (aiIdx + 1) + " | total=" + (humanCount + currentAiCount + 1) + "/" + maxPlayers)
        renderSlots()
        broadcastSlotState()
      })
    })
  ...
  const renderLanMapList = () => {
    ...
    var bodyEl = body
    bodyEl.innerHTML = ""
    profiles.forEach(function (profile: { id: string; icon?: string; name: string; desc: string; params: { maxRounds: number; directTakeRatio: number } }) {
  ...
  const openLanCarryItemPicker = () => {
    ...
    var available = shopItems.map(function (def: { id: string; name: string; icon: string }) {
      var storageKey = bridge2.getItemStorageKey(def.id)
      return { id: def.id, name: def.name, icon: def.icon, count: inventory[storageKey] || 0 }
    }).filter(function (item: { count: number }) { return item.count > 0 })
    ...
    var renderPicker = function () {
      ...
      panel.innerHTML =
        ...
        available.map(function (item: { id: string; icon: string; name: string; count: number }) {
      ...
      if (confirmBtn) confirmBtn.addEventListener("click", function () {
        lanCarryItems = available.filter(function (item: { id: string }) { return pickerSelected.has(item.id) })
          .map(function (item: { id: string; name: string; icon: string }) { return { id: item.id, name: item.name, icon: item.icon } })
