...
export const LanEventsMixin: ThisType<WarehouseSceneThis> = {
  ...
  bindLanEvents(bridge: LanBridge, ctx: LanEventsCtx) {
    ...
    bridge.on("game:init", (msg: RoomMessage) => {
      ...
      if (this.lanIsHost) {
        ...
        this.lanAiPlayers = aiPlayersFromMsg.length > 0
          ? aiPlayersFromMsg.map((ai: { id: string; name: string; llm?: boolean }) => ({ id: ai.id, name: ai.name, isAI: true, isHost: false, llm: ai.llm }))
          : [];
        ...
        this.lanAiPlayers = aiPlayersFromMsg.map((ai: { id: string; name: string; llm?: boolean }) => ({ id: ai.id, name: ai.name, isAI: true, isHost: false, llm: ai.llm }));
        this.lanAiPlayers.forEach((ai) => {
          this.lanPlayers.push(ai as LanPlayer);
        });
      }
    ...
    bridge.on("round:start", (msg: RoomMessage) => {
      if (!this.lanIsHost) {
        this.lanOnRoundStart(msg as unknown as { round: number; currentBid?: number; ts?: number; roundSeconds?: number });
      } else {
    ...
    bridge.on("all-bids-in", (msg: RoomMessage) => {
      if (!this.lanIsHost) return;
      this.lanOnAllBidsIn(msg as unknown as Record<string, unknown>).catch((e) => this.writeLog("AI行动异常：" + (e && e.message ? e.message : e)));
    });
