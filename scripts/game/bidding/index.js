...
(function setupMobaoBidding(global) {
  ...
  const BiddingMixin = {
    ...
    async resolveRoundBids(reason = "manual", forceSettle = false) {
      console.log(`[resolveRoundBids] reason=${reason}, forceSettle=${forceSettle}, settled=${this.settled}, roundResolving=${this.roundResolving}, round=${this.round}, isLanMode=${this.isLanMode}`);
      ...
      try {
        ...
        console.log("[resolveRoundBids] final roundBids:", roundBids.map(b => ({ playerId: b.playerId, bid: b.bid })));
    ...
    buildRoundBids() {
      ...
      aiPlayers.forEach((player) => {
        ...
        console.log(`[buildRoundBids] ${player.id} aiLlmPlan:`, plan ? { failed: plan.failed, hasBidDecision: plan.hasBidDecision, bid: plan.bid, canUseLlm: this.canUseLlmDecisionForPlayer(player.id) } : "null");
        ...
        console.log(`[buildRoundBids] ${player.id} LLM bid override: ${aiBidMap[player.id]} -> ${normalizedBid} (wallet=${wallet})`);
