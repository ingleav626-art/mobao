 ...
 */
const { getUnlockedCharacters } = window.CharacterData
...
export const CharacterSelectMixin = {
  ...
  _loadCarryItems() {
    try {
      ...
      this._carryItems = parsed.filter((i) => i && typeof i.id === "string").slice(0, this._MAX_CARRY_ITEMS)
    } catch (_e) {
      this._carryItems = []
    }
  },
  ...
  selectCharacter(characterId) {
    const char = window.CharacterData.getCharacterById(characterId)
    if (!char) return
...
window.MobaoLobby.CharacterSelectMixin = CharacterSelectMixin
