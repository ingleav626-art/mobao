...
(function setupMobaoAiIntel(global) {
  ...
  const AiIntelMixin = {
    ...
    initAiIntelSystems() {
      ...
      const allCharacters = (window.CharacterData && window.CharacterData.CHARACTERS) || [];
    ...
    revealPrivateIntelFully(playerId, { count, sortStrategy, category, allowCategoryFallback }) {
      const pool = this.ensureAiPrivateIntel(playerId);
      const unrevealed = this.items.filter((item) => !pool.knownOutlineIds.has(item.id) || !pool.knownQualityIds.has(item.id));
    ...
    async processAiIntelActions() {
      ...
      console.log(`[processAiIntelActions] ${batchId} START, aiPlayers: ${aiPlayers.length}, players: ${aiPlayers.map(p => p.id).join(",")}`);

      return Promise.all(aiPlayers.map(async (player) => {
        try {
    ...
    async processSingleAiIntelAction(player, plan, llmPlan, roundProgress, batchId, batchStartTime) {
      ...
      console.log(`[processSingleAiIntelAction] ${player.id}-${startTime} START, delay from batch start: ${startTime - batchStartTime}ms`);
      console.log(`[processSingleAiIntelAction] ${player.id} plan:`, plan ? { actionType: plan.actionType, actionId: plan.actionId, decisionSource: plan.decisionSource, lockedByLlm: plan.lockedByLlm } : "null");
      console.log(`[processSingleAiIntelAction] ${player.id} llmPlan:`, llmPlan ? { failed: llmPlan.failed, hasBidDecision: llmPlan.hasBidDecision, bid: llmPlan.bid, actionId: llmPlan.actionId } : "null");
      ...
      console.log(`[processSingleAiIntelAction] ${player.id} executeAiIntelAction result:`, { ok: result.ok, actionType: plan.actionType, actionId: plan.actionId, message: result.message });
      ...
      if (!result.ok && plan.actionType !== "none" && llmBidReady && this.canUseLlmDecisionForPlayer(player.id)) {
          ...
          if (correctionResult.ok) {
            ...
            if (correctionPlan.actionType !== "none" && correctionPlan.actionId !== "none") {
              ...
              if (this.canUseLlmDecisionForPlayer(player.id)) {
                ...
                console.log(`[processSingleAiIntelAction] ${player.id} correction followup result:`, followup ? { ok: followup.ok, failed: followup.failed, hasBidDecision: followup.hasBidDecision, bid: followup.bid } : "null");
        ...
        console.log(`[processSingleAiIntelAction] ${player.id}-${startTime} END (error correction path), elapsed: ${Date.now() - startTime}ms`);
        return;
      }

      if (!result.ok || plan.actionType === "none") {
        console.log(`[processSingleAiIntelAction] ${player.id}-${startTime} END (no action), elapsed: ${Date.now() - startTime}ms`);
        return;
      }
      ...
      if (llmBidReady && llmPlan.actionId === plan.actionId) {
        ...
        if (this.canUseLlmDecisionForPlayer(player.id)) {
          ...
          console.log(`[processSingleAiIntelAction] ${player.id} followup result:`, followup ? { ok: followup.ok, failed: followup.failed, hasBidDecision: followup.hasBidDecision, bid: followup.bid } : "null");
          if (followup && !followup.failed && followup.hasBidDecision) {
            llmPlan.bid = followup.bid;
            llmPlan.hasBidDecision = true;
            llmPlan.thought = followup.thought || llmPlan.thought;
