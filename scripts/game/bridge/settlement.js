  ...
  function createSettlementBridge(deps) {
    ...
    const methods = {
      ...
      async revealAllArtifactsForSettlement() {
        ...
        this.setSettlementProgress(`正在揭示藏品 ${revealedCount}/${totalCount}，点击游戏区可跳过。`, revealedCount / totalCount);
        ...
        for (let i = 0; i < revealQueue.length; i += 1) {
          ...
          revealedCount += 1;
          this.updateSettlementPanelMetrics(revealedValue, revealedValue - this.settlementSession.winnerBid);
          this.setSettlementProgress(`正在揭示藏品 ${revealedCount}/${totalCount}：${item.name}`, revealedCount / totalCount);
      ...
      playSettlementFinalEffect(winnerProfit) {
        ...
        const hasLegendaryItems = this.items && this.items.some(function (it) { return it.qualityKey === "legendary"; });
      ...
      enterSettlementPage(winnerPlayer, winnerBid, reasonText) {
        ...
        if (typeof MobaoAnimations !== "undefined") {
          ...
          // 各个 meta 行
          metaElements.forEach(function (el) { staggerTargets.push(el); });
          ...
          // 按钮
          actionButtons.forEach(function (el) { staggerTargets.push(el); });
      ...
      exitSettlementPage() {
        ...
        this.dom.settlementPage.classList.remove("settle-slide-in", "settle-glow", "settle-glow-enhanced", "settle-screen-shake");
        const profitEl = this.dom.settleWinnerProfit;
        if (profitEl) {
          profitEl.classList.remove("profit-animate-bounce", "profit-animate-shake", "profit-animate-spark");
        }
