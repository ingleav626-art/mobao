  ...
  function createSceneLlmBridge(deps) {
    ...
    const methods = {
      ...
      showAiConversationMessages() {
        ...
        Object.keys(messages).sort().forEach((playerId) => {
      ...
      fillLlmSettingsForm(values) {
        ...
        const independentModelCheckbox = this.dom.settingLlmIndependentModelEnabled || document.getElementById("setting-llmIndependentModelEnabled");
        ...
        if (independentReflectionCheckbox) {
          independentReflectionCheckbox.checked = source.independentReflectionEnabled !== undefined ? Boolean(source.independentReflectionEnabled) : true;
        }
      ...
      readLlmSettingsForm() {
        ...
        const endpointInput = document.getElementById("setting-llmEndpoint");
        const independentModelCheckbox = this.dom.settingLlmIndependentModelEnabled || document.getElementById("setting-llmIndependentModelEnabled");
        console.log("[readLlmSettingsForm] independentModelCheckbox:", independentModelCheckbox ? "found" : "not found", "checked:", independentModelCheckbox ? independentModelCheckbox.checked : "N/A");
        const independentReflectionCheckbox = document.getElementById("setting-llmIndependentReflectionEnabled");
        ...
        return {
          ...
          independentReflectionEnabled: independentReflectionCheckbox
            ...
            : (currentSettings.independentReflectionEnabled !== undefined ? currentSettings.independentReflectionEnabled : true)
        };
      },
      ...
      buildAiLlmRoundPayload(player) {
        ...
        const availableSkills = SKILL_DEFS
        ...
        const availableItems = ITEM_DEFS
      ...
      canUseLlmDecision() {
        ...
        if (!settings || !settings.enabled || !provider) {
          console.log("[canUseLlmDecision] false: settings=", settings ? { enabled: settings.enabled } : "null", "provider=", provider ? provider.id : "null");
          return false;
        }
        ...
        console.log("[canUseLlmDecision] false: no apiKey and not proxy endpoint on desktop, endpoint:", endpoint, "isNative:", isNative);
        return false;
      },
      ...
      getAiModelConfigForPlayer(playerId) {
        const settings = typeof this.getLlmSettings === "function" ? this.getLlmSettings() : LLM_SETTINGS;
        console.log("[getAiModelConfigForPlayer] playerId:", playerId, "settings.independentModelEnabled:", settings ? settings.independentModelEnabled : "no settings");
        ...
        if (typeof this.getAiModelConfig === "function") {
          ...
          console.log("[getAiModelConfigForPlayer] got config for aiIndex", aiIndex, ":", config ? { apiKey: config.apiKey ? "(已设置)" : "(空)", endpoint: config.endpoint, model: config.model } : null);
      ...
      normalizeAiLlmPlan(playerId, decision, rawContent, options = {}) {
        ...
        console.log(`[normalizeAiLlmPlan] ${playerId} decision:`, decision ? { bid: bidRaw, skill: skillRaw, item: itemRaw, hasBidDecision, normalizedBid: bid, actionType, actionId } : "null/empty");
      ...
      async requestAiLlmPlan(player, options = {}) {
        ...
        console.log(`[requestAiLlmPlan] ${requestId} START, player: ${player.id}, batchId: ${batchId}, delay from batch start: ${requestStartTime - batchStartTime}ms`);
        ...
        if (options.requestStage === "followup-after-tool") {
          payload = this.buildAiFollowupRoundPayload(player, options.followupContext || {}, options.followupToolSummary || "");
        } else if (isFirstRound) {
        ...
        const firstRoundBlocks = isFirstRound
        ...
        try {
          ...
          console.log("[requestAiLlmPlan] base settings:", { enabled: settings.enabled, apiKey: settings.apiKey ? "(已设置)" : "(空)", endpoint: settings.endpoint, model: settings.model });
          ...
          try {
            ...
            console.log("[requestAiLlmPlan] aiModelConfig:", aiModelConfig ? { apiKey: aiModelConfig.apiKey ? "(已设置)" : "(空)", endpoint: aiModelConfig.endpoint, model: aiModelConfig.model } : null);
            if (aiModelConfig) {
              settings = {
                ...settings,
              ...
              console.log("[requestAiLlmPlan] merged settings:", { apiKey: settings.apiKey ? "(已设置)" : "(空)", endpoint: settings.endpoint, model: settings.model });
            }
          } catch (e) {
          ...
          console.log(`[requestAiLlmPlan] ${requestId} CALLING requestChat, model: ${settings.model}, elapsed so far: ${chatStartTime - requestStartTime}ms`);
        ...
        if (result && Array.isArray(result.signals) && result.signals.length > 0) {
          ...
          result.signals.forEach((signal) => {
            ...
            if (signal.mode === "quality") {
              const qualityConfig = (global.ArtifactData && global.ArtifactData.QUALITY_CONFIG) || {};
      ...
      async prepareAiLlmRoundPlans() {
        ...
        console.log(`[prepareAiLlmRoundPlans] ${batchId} START, activePlayers: ${activePlayers.length}, players: ${activePlayers.map(p => p.id).join(",")}`);

        const plans = await Promise.all(activePlayers.map((player) => this.requestAiLlmPlan(player, { batchId, batchStartTime })));

        const batchEndTime = Date.now();
        const batchElapsed = batchEndTime - batchStartTime;
        console.log(`[prepareAiLlmRoundPlans] ${batchId} END, total elapsed: ${batchElapsed}ms, avg per player: ${Math.round(batchElapsed / activePlayers.length)}ms`);
      ...
      async processAiDecisions() {
        ...
        console.log("[processAiDecisions] aiPlayers:", aiPlayers.map(p => p.id));
        console.log("[processAiDecisions] activePlayers:", activePlayers.map(p => p.id));
        console.log("[processAiDecisions] disabledPlayers:", disabledPlayers.map(p => p.id));
        aiPlayers.forEach(p => {
          ...
          console.log(`[processAiDecisions] ${p.id} canUseLlmDecisionForPlayer=${canUse}, isAiLlmEnabledForPlayer=${isEnabled}, globalEnabled=${globalEnabled}`);
        });
        ...
        console.log(`[processAiDecisions] ${batchId} START, activePlayers: ${activePlayers.length}, disabledPlayers: ${disabledPlayers.length}`);
        ...
        aiPlayers.forEach((player) => {
          ...
          console.log(`[processAiDecision] ${player.id}-${startTime} START, delay from batch start: ${startTime - batchStartTime}ms`);
          ...
          const taskPromise = (async () => {
            try {
              ...
              if (!this.isLanMode && !this.roundResolving && !this.settled && !this.roundPaused) {
                ...
                console.log(`[processAiDecision] ${player.id} finally: areAllPlayersBidReady=${allReady}, roundResolving=${this.roundResolving}, settled=${this.settled}`);
                if (allReady) {
