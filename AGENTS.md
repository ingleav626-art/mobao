...
- **测试是用来抓真实 bug 的，不是用来通过的**。如果测试通过但 bug 存在，测试就是错的，必须重写。不为通过而测。
- **每个测试必须写明预期**：用注释写"应该发生什么、为什么"。预期错了，测了也白测。**不要自己瞎猜规则**--游戏有独立的胜负判断体系，不能假设"最高价赢"之类。如果单机正常联机出错，预期应是"联机结果与单机一致（复用同一逻辑）"，即**奇偶测试**：同样输入，联机==单机。
- **行为测试测用户可见结果，不测内部函数调用**。测"联机和单机结算结果一致"（用户关心），不测"resolveRoundBids 被调用了"（用户不关心）。
- **写测试前先问：这个测试能不能抓到 XXX bug？** 不能就是废测试。例：能不能抓到"联机结算和单机不一致"？不能就重写。
- **联机必须复用单机逻辑，不重复实现**。单机正常的逻辑，联机应直接调用，不该另写一套。如果联机有单机没有的 bug，说明没复用，先查是否重复实现。
- **禁止一刀切**：重置/赋值方法必须区分语义（resetLanState 保留连接 vs disconnectLan 断开），不能图省事全重置。每个 reset 方法注释写明"重置什么、保留什么、为什么"。
- **子代理不会自己思考，大脑（主代理）必须给精确指令 + 预期**。指令里漏了边界条件，子代理就会犯错。
