...
| 阶段 | 原始目标 | 当前实际 | 状态 |
|------|---------|---------|------|
| 起点 | 7% / 458 用例 | 7% / 458 用例 | ✅ 基线 |
| Phase 1 | 25% / 540 用例 | 13.17% / 539 用例 | 🟡 覆盖率未达标，用例数达标 |
| Phase 2 | 40% / 580 用例 | 13.4% / 885 用例 | 🟡 用例数远超目标，覆盖率受大文件稀释 |
| Phase 3 | 60% / 650+ 用例 | 13.7% / 972 用例 | 🟡 用例数远超目标，覆盖率受大文件稀释 |

> **说明**：用例数已远超原计划（972 vs 650+），但覆盖率提升缓慢。原因是项目新增了大量未测试的代码（如 lan 模块、character-select.ts 等），稀释了覆盖率。下一步应聚焦"高行数低覆盖"模块，而非继续增加已测试模块的边界用例。
...
1. **Phase 1** - 纯函数补充（零 mock，最高 ROI）✅ 已完成
2. **Phase 2** - localStorage mock 测试 ✅ 已完成
3. **Phase 3** - 扩展已有测试 + 深度覆盖 ✅ 已完成
4. **Phase 4**（新增）- 纯函数提取 + 大文件拆分，提升覆盖率 ROI

---

## Phase 4：纯函数提取 + 大文件拆分（进行中）

> **背景**：Phase 1-3 用例数已远超目标，但覆盖率受大文件稀释。Phase 4 转向"拆分大文件 + 提取纯函数"，通过减少未测试代码量来提升覆盖率。

### 已完成

| 文件 | 操作 | 效果 |
|------|------|------|
| `ai/intel.ts` | 提取 7 个纯函数 | +32 测试用例 |
| `llm/core/llm-decision.ts` | 提取 8 个纯函数 | +39 测试用例 |
| `scripts/game/scene/scene-init.ts` | 拆分 bindDomEvents 为 7 个事件模块 | 降低单文件复杂度 |
| `scripts/game/warehouse/index.ts` | 拆分 3 个 Mixin 到 core.ts/reveal.ts/preview.ts | 1306 行 -> 108 行 |

### 下一步候选

| 文件 | 当前行数 | 操作 | 预计收益 |
|------|---------|------|---------|
| `llm/core/llm-decision.ts` | 1627 | 继续提取纯函数或拆分 Mixin | 高（最大文件） |
| `llm/core/llm-manager.ts` | 1186 | 提取配置/注册逻辑为纯函数 | 中 |
| `lobby/character-select.ts` | 1194 | 提取 UI 逻辑为纯函数 | 中 |
| `lan/client/lan-bridge.ts` | 573 | 提取消息解析为纯函数 | 低 |

---

## 预期结果（更新）

| 指标 | 起点 | Phase 1-3 实际 | Phase 4 目标 |
|------|------|---------------|-------------|
| 测试文件 | 22 | 40 | 42+ |
| 测试用例 | 458 | 972 | 1050+ |
| 语句覆盖率 | 7% | 13.7% | **18%+** |
| 纯函数覆盖率 | 40%~96% | 70%~100% | 80%~100% |
